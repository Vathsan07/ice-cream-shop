...
const products = [
  {
    ...
    desc: 'Rich chocolate made with cocoa.',
    sizes: [
      { label: '200ml', price: 60 },
...
export default function IceCreamShop() {
  ...
  const genOrderId = () =>
    'ORD-' + Date.now().toString(36).toUpperCase().slice(0, 8);
  ...
  // ───────────────────────────────────────────────
  // SEND ORDER → MAKE.COM (Vercel-friendly)
  // ───────────────────────────────────────────────
  const handlePlaceOrder = async (e) => {
    ...
    try {
      const MAKE_WEBHOOK = "https://hook.us2.make.com/ox8dtbe946ulga5g5b7b3p28mnt31ml7";
      ...
      <aside className="order-panel">
              ...
              <div key={key} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <div>
                  <div style={{ fontWeight: 600 }}>
                    {prod?.name} <span style={{ fontWeight: 400, fontSize: 12 }}>({size?.label})</span>
                  </div>
                  <div className="small text-muted">Rs.{size?.price} × {qty}</div>
                </div>
                <div style={{ fontWeight: 700 }}>Rs.{size?.price * qty}</div>
              </div>
            );
          })}
        </div>
        ...
        {orderResult && orderResult.ok && (
          <div className="order-success" style={{ marginTop: 12 }}>
            ...
            <div className="small text-muted">Order sent successfully.</div>
          </div>
        )}
      </aside>
      ...
      {showCheckout && (
        <div style={{
          ...
          <form onSubmit={handlePlaceOrder} style={{
            width: 420, background: '#fff', padding: 20,
            ...
            <div style={{
              ...
              <button type="button" className="btn btn-secondary" onClick={() => setShowCheckout(false)} disabled={processing}>
                Cancel
              </button>
